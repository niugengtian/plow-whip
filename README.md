...
发布 Task 必须在 `submit` 时显式加 `--release-branch`；该标记会进入 Git workflow。GitHub E2E 完成后，Worker 通过受租约保护的 `task progress --release-gate-report '<JSON>'` 写入验证证据，发布进程在最终 fast-forward `main` 前再强制校验。未标记的普通 Task 不触发该闸门。
