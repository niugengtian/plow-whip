...
def render_handbook(data: dict) -> str:
    ...
    lines += [
        ...
        "## 入口与状态迁移",
        "",
        "- 任务通过 `submit` 投递；严格模式只接受绑定的人机控制面（macOS 为 Codex Desktop，Linux/Windows 为交互式终端），旧模式仍可由其他入口提交。Agent 每次开始或恢复工作前通过 `start --agent ... --json` 获取有界上下文。",
        ...
        "- macOS 人工控制权同时校验绑定 Thread 与本机 Desktop App 进程父链；Linux/Windows 校验交互式 TTY 与绑定终端会话。环境变量本身不构成授权。严格模式只调度可传递租约的 CLI Driver，zellij 仅保留旧模式兼容。",
    ...
    lines += [
        ...
        "- 默认可信人工控制面在 macOS 是 Codex Desktop，在 Linux/Windows 是首次绑定的交互式终端；默认 Cursor Desktop 仅为 `schedulable=false` 的观察身份，不拥有 Task，也不能在严格项目中提交、确认或答复决策。",
        "- 严格项目的控制命令必须由当前绑定的人机控制会话授权；Worker 不继承 Desktop origin/thread，其重定向标准流也不满足交互终端校验。macOS 切换控制会话必须显式执行 Desktop 同步。",
